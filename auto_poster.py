...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["알바몬 기업회원(사장님) 가입 - 2만원 쿠폰 지급", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 소상공인 창업 컨설턴트이자 프랜차이즈 경영 지도사입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "business owner shop",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'business owner shop')
